...
::: {.callout-tip}
These models do not include an interaction between $X_1$ and $X_2$. We might instead have identified that there is spurious association between $Y$ and $X_1$ that is corrected when we include $X_2$. 
:::
...
**Answer -** The regression results suggest that there will be higher voter turnout rates being in a non-Southern state, having election day registration, and having a high percentage of the population that graduated high school. 

::: {.callout-tip}
Be careful with your language. You are suggesting a causal relationship. We cannot use regression to test causal relationships. It is better to say that these variables are *associated* with changes in voter turn-out rates.
:::

The regression results show that non-Southern states have a positive association with voting turnout rates since there is a 5.546 increase, meaning this effect is statistically significant at a p-value of 0.001. States that have election day registration is connected with a 5.792 increase in the voter turnout rate with a statistical significance at the p-value of 0.001. There is also an increase in the voter turnout of 0.101 in the percentage of the population that graduated high school at the p-value of 0.01. $R^2$ is 0.281 which indicates the proportion of variance in the dependent variable that is explained by the independent variable. The F statistic of 46.049 is the statistic tests of the significance of the overall model.

Interpreting the constant term on its own does not seem meaningful since the constant term is a representative of the expected turnout rate when the other independent variables are zero. If the independent variables are dummy variables, like nonSouth and edr, then this creates a problem.

::: {.callout-tip}
Binary variables often produce meaningful intercept coefficients. When `nonSouth` is equal to zero, we are looking at a Southern state. When `edr` is equal to zero, we are looking at a state that does not allow for election-day registration. 

Our intercept coefficient isn't meaningful on its own because we are never going to see a state that has a high school graduation rate of zero percent. 
:::

Interpreting the constant without having a significant consideration of the other variables can lead to misinterpretation.
...
::: {.callout-tip}
To answer this question, you need to fit your null model. In this model, `nonSouth` and `edr` have no effect on `vote`. To fit this model, you can simply exclude those variables. 

You can then compare the predictive power of your full model with this null model. Are they significantly different from one another? If so, you can reject the null hypothesis that these variables do not contribute to the predictive power of your model. 
:::
...
**Answer -** I picked these variables because I wanted to know how support for gay rights, at different levels, and how the percentage of the population being conservative depended on high religiosity. 

::: {.callout-tip}
Your phrasing suggests that you consider religiosity to be the independent variable: it drives individuals' level of support for gay rights. 
:::

As an Americanist in the department, one of my research interest is religion. I hypothesize that when there is a high level of religiosity, there will also be large number of conservatives in the population. 

::: {.callout-tip}
I (representing your very silly reader) need all jargon explained. What is religiosity? How do you measure it here?
:::

::: {.callout-tip}
This is not really a hypothesis. Your hypothesis needs to describe what you expect to be the effect of your independent variable on your dependent variable. 

For example, you might hypothesize that an increase in the proportion of a state's population that identify as conservative is associated with an increase in the proportion of that population that identify as religious. 
:::

Also, with a high level of religiosity also comes a large drop in the number of people who have high public support of gay rights.
